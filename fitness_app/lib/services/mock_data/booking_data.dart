import '../../models/booking_model.dart';

final List<Booking> userBookings = [
  // Бронирования на сегодня (22 сентября)
  Booking(
    id: 'booking_001',
    userId: 'user_001',
    type: BookingType.tennisCourt,
    startTime: DateTime(2025, 9, 22, 9, 0),
    endTime: DateTime(2025, 9, 22, 10, 0),
    title: 'Теннисный корт',
    description: 'Корт 2 - Грунт',
    status: BookingStatus.confirmed,
    price: 1200,
    courtNumber: 'Корт 2',
    createdAt: DateTime(2025, 9, 14),
  ),
  Booking(
    id: 'booking_002',
    userId: 'user_001',
    type: BookingType.groupClass,
    startTime: DateTime(2025, 9, 22, 17, 0),
    endTime: DateTime(2025, 9, 22, 18, 0),
    title: 'Пилатес',
    description: 'С Еленой Сидоровой',
    status: BookingStatus.confirmed,
    price: 550,
    trainerId: 'trainer_003',
    className: 'Пилатес',
    createdAt: DateTime(2025, 9, 18),
  ),

  // Бронирования на завтра (23 сентября)
  Booking(
    id: 'booking_003',
    userId: 'user_001',
    type: BookingType.personalTraining,
    startTime: DateTime(2025, 9, 23, 16, 0),
    endTime: DateTime(2025, 9, 23, 17, 0),
    title: 'Теннис с тренером',
    description: 'С Марией Ивановой',
    status: BookingStatus.confirmed,
    price: 2000,
    trainerId: 'trainer_001',
    className: 'Теннис с тренером',
    createdAt: DateTime(2025, 9, 18),
  ),
  Booking(
    id: 'booking_004',
    userId: 'user_001',
    type: BookingType.groupClass,
    startTime: DateTime(2025, 9, 23, 19, 0),
    endTime: DateTime(2025, 9, 23, 20, 0),
    title: 'Функциональный тренинг',
    description: 'С Дмитрием Петровым',
    status: BookingStatus.confirmed,
    price: 650,
    trainerId: 'trainer_002',
    className: 'Функциональный тренинг',
    createdAt: DateTime(2025, 9, 19),
  ),

  // Бронирования на 24 сентября
  Booking(
    id: 'booking_005',
    userId: 'user_001',
    type: BookingType.groupClass,
    startTime: DateTime(2025, 9, 24, 12, 0),
    endTime: DateTime(2025, 9, 24, 13, 0),
    title: 'Стретчинг',
    description: 'С Еленой Сидоровой',
    status: BookingStatus.confirmed,
    price: 500,
    trainerId: 'trainer_003',
    className: 'Стретчинг',
    createdAt: DateTime(2025, 9, 20),
  ),
  Booking(
    id: 'booking_006',
    userId: 'user_001',
    type: BookingType.locker,
    startTime: DateTime(2025, 9, 24, 8, 0),
    endTime: DateTime(2025, 9, 24, 22, 0),
    title: 'Аренда шкафчика',
    description: 'Шкафчик A-103',
    status: BookingStatus.awaitingPayment,
    price: 150,
    lockerNumber: 'A-103',
    createdAt: DateTime(2025, 9, 23),
  ),

  // Бронирования на 25 сентября
  Booking(
    id: 'booking_007',
    userId: 'user_001',
    type: BookingType.groupClass,
    startTime: DateTime(2025, 9, 25, 10, 0),
    endTime: DateTime(2025, 9, 25, 12, 0),
    title: 'Кроссфит',
    description: 'С Дмитрием Петровым',
    status: BookingStatus.confirmed,
    price: 700,
    trainerId: 'trainer_002',
    className: 'Кроссфит',
    createdAt: DateTime(2025, 9, 20),
  ),
  Booking(
    id: 'booking_008',
    userId: 'user_001',
    type: BookingType.tennisCourt,
    startTime: DateTime(2025, 9, 25, 19, 0),
    endTime: DateTime(2025, 9, 25, 20, 30),
    title: 'Теннисный корт',
    description: 'Корт 1 - Хард',
    status: BookingStatus.confirmed,
    price: 2250,
    courtNumber: 'Корт 1',
    createdAt: DateTime(2025, 9, 20),
  ),

  // Бронирования на 26 сентября
  Booking(
    id: 'booking_009',
    userId: 'user_001',
    type: BookingType.personalTraining,
    startTime: DateTime(2025, 9, 26, 10, 0),
    endTime: DateTime(2025, 9, 26, 11, 0),
    title: 'Силовая тренировка',
    description: 'С Дмитрием Петровым',
    status: BookingStatus.confirmed,
    price: 1800,
    trainerId: 'trainer_002',
    className: 'Силовая тренировка',
    createdAt: DateTime(2025, 9, 21),
  ),
  Booking(
    id: 'booking_010',
    userId: 'user_001',
    type: BookingType.groupClass,
    startTime: DateTime(2025, 9, 26, 18, 30),
    endTime: DateTime(2025, 9, 26, 19, 30),
    title: 'Йога для начинающих',
    description: 'С Еленой Сидоровой',
    status: BookingStatus.awaitingPayment,
    price: 450,
    trainerId: 'trainer_003',
    className: 'Йога для начинающих',
    createdAt: DateTime(2025, 9, 22),
  ),

  // Бронирования на 27 сентября (прошедшие даты)
  Booking(
    id: 'booking_011',
    userId: 'user_001',
    type: BookingType.tennisCourt,
    startTime: DateTime(2025, 9, 19, 14, 0), // Прошедшая дата
    endTime: DateTime(2025, 9, 19, 15, 30),
    title: 'Теннисный корт',
    description: 'Корт 3 - Хард',
    status: BookingStatus.completed,
    price: 2250,
    courtNumber: 'Корт 3',
    createdAt: DateTime(2025, 9, 12),
  ),
  Booking(
    id: 'booking_012',
    userId: 'user_001',
    type: BookingType.personalTraining,
    startTime: DateTime(2025, 9, 20, 16, 0), // Прошедшая дата
    endTime: DateTime(2025, 9, 20, 17, 0),
    title: 'Персональная тренировка',
    description: 'С Марией Ивановой',
    status: BookingStatus.completed,
    price: 1500,
    trainerId: 'trainer_001',
    className: 'Персональная тренировка',
    createdAt: DateTime(2025, 9, 10),
  ),

  // Бронирования на 28 сентября
  Booking(
    id: 'booking_013',
    userId: 'user_001',
    type: BookingType.groupClass,
    startTime: DateTime(2025, 9, 21, 9, 0), // Прошедшая дата
    endTime: DateTime(2025, 9, 21, 10, 0),
    title: 'Утренняя йога',
    description: 'С Еленой Сидоровой',
    status: BookingStatus.completed,
    price: 500,
    trainerId: 'trainer_003',
    className: 'Утренняя йога',
    createdAt: DateTime(2025, 9, 12),
  ),
  Booking(
    id: 'booking_014',
    userId: 'user_001',
    type: BookingType.personalTraining,
    startTime: DateTime(2025, 9, 28, 19, 0),
    endTime: DateTime(2025, 9, 28, 20, 0),
    title: 'Спортивный массаж',
    description: 'С Анной Ковалевой',
    status: BookingStatus.awaitingPayment,
    price: 3000,
    trainerId: 'trainer_massage',
    className: 'Спортивный массаж',
    createdAt: DateTime(2025, 9, 24),
  ),

  // Бронирования на 29 сентября
  Booking(
    id: 'booking_015',
    userId: 'user_001',
    type: BookingType.tennisCourt,
    startTime: DateTime(2025, 9, 29, 10, 0),
    endTime: DateTime(2025, 9, 29, 11, 30),
    title: 'Теннисный корт',
    description: 'Корт 2 - Грунт',
    status: BookingStatus.confirmed,
    price: 1800,
    courtNumber: 'Корт 2',
    createdAt: DateTime(2025, 9, 25),
  ),
  Booking(
    id: 'booking_016',
    userId: 'user_001',
    type: BookingType.groupClass,
    startTime: DateTime(2025, 9, 29, 18, 0),
    endTime: DateTime(2025, 9, 29, 19, 0),
    title: 'HIIT тренировка',
    description: 'С Дмитрием Петровым',
    status: BookingStatus.confirmed,
    price: 600,
    trainerId: 'trainer_002',
    className: 'HIIT тренировка',
    createdAt: DateTime(2025, 9, 26),
  ),
];